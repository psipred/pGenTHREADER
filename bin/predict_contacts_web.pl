...
if(-e $FILE.".mtx")
{
	`/webdata/binaries/current/mtx2con/mtx2con $FILE.mtx > $FILE.cons`;
}

if(-e $FILE.".contactcons")
{
...
sub output_predictions
{
	...
	my $fhIn = new FileHandle($file.".contactcons","r");
	my $seq_length = 0;
...
sub read_libsvm
{
	...
	while(my $line = $fhIn->getline)
	{
		...
		if($line =~ /^1\s(.+)\s.+/)
		{
			...
			if($hPredictions->{$count}{PROB} >= 0.6)
			{
...
sub get_data
{
	...
	my $fasta_file = $file_name;
	$fasta_file =~ s/\.omg//;
	my $aResidues = read_fsa($dir."/".$fasta_file.".fasta");
	if($aResidues =~ /^NO/){return("NO")}
	my $aConsensus = read_cons($dir."/".$file_name.".cons");
	if($aConsensus =~ /^NO/){return("NO")}	
	my $hContact_con = read_contactcons($dir."/".$file_name.".contactcons");
